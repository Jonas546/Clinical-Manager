...
type
  ...
  { Tt_relatorio_convenio_pdf2 }

  Tt_relatorio_convenio_pdf2 = class(TForm)
...
var
  t_relatorio_convenio_pdf2: Tt_relatorio_convenio_pdf2;
  a_guia:integer;
  t_guia,pref:String;
...
procedure Tt_relatorio_convenio_pdf2.RLConveniosBeforePrint(Sender: TObject;
  var PrintIt: boolean);
begin
     ...
     with t_lancamento_convenio do
       ...
       if rb_guias.Checked=true then
          begin
            lb_comprovante.Caption:='Guias';
          end;

       if rb_online.Checked=true then
          begin
            lb_comprovante.Caption:='On Line';
          end;

       if rb_planilha.Checked=true then
          begin
            lb_comprovante.Caption:='Planilhas';
          end;

       if rb_etc.Checked=true then
          begin
            lb_comprovante.Caption:='Planilhas, Guias, Etc';
          end;

       if rb_todos_comprovantes.Checked=true then
          begin
            lb_comprovante.Caption:='Todos os Comprovantes';
          end;
       ...
       if rb_especial1.Checked=true then
          begin
           ...
           DMOS.query_convenios.SQL.Add('where c.cod_paciente = p.id and m.nome = :v1 and c.forma_pagamento = ''Convênio'' and lc.cod_guia = c.guia  and p.plano = l.plano and p.convenio = v.convenio and c.cod_medico = m.id');
           ...
           if rb_atendimento.Checked=true then
              begin
                DMOS.query_convenios.SQL.Add('and data_consulta between (cast(:d1 as Date)) and (cast(:d2 as date))');
              end;

           if rb_entrega.Checked=true then
              begin
                DMOS.query_convenios.SQL.Add('and data_entrega between (cast(:d1 as Date)) and (cast(:d2 as date))');
              end;
           ...
           if ck_todos.Checked=true then
              begin
                DMOS.query_convenios.SQL.Add('and lc.status in(''Enviado'',''Não Enviado'')');
              end;
...
procedure Tt_relatorio_convenio_pdf2.RLConveniosPageEnding(Sender: TObject);
begin
