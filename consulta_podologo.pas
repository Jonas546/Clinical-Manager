...
uses
  ...
  Menus, StdCtrls, DbCtrls, ExtCtrls, Grids;

type

  { Tt_consulta_podologo }

  Tt_consulta_podologo = class(TForm)
    btn_salvar: TButton;
...
var
  t_consulta_podologo: Tt_consulta_podologo;
...
{$R *.lfm}
uses dm, home, podologo_agenda, podologo_orcamento, podologo_materiais;
...
begin
  ...
  DMC.query_podologo.Close;
  DMC.query_podologo.ParamByName('v1').AsInteger:=key;
  DMC.query_podologo.ParamByName('v2').AsInteger:=guia;
  DMC.query_podologo.Open;

end;

procedure Tt_consulta_podologo.FormCreate(Sender: TObject);
begin
...
procedure Tt_consulta_podologo.FormClose(Sender: TObject;
...
procedure Tt_consulta_podologo.btn_salvarClick(Sender: TObject);
var
...
begin
  ...
  if (txt_procedimento.Text <> '') and (cb_tipo.Text <> '') then
   begin
     ...
     a_guia:=strtoint(t_podologo_agenda.db_txt_guia.Text);
...
procedure Tt_consulta_podologo.Button1Click(Sender: TObject);
var
  especialidades: String;
  a_medico, a_paciente, a_guia:integer;
begin
...
procedure Tt_consulta_podologo.cb_lista_procedimentoChange(Sender: TObject);
begin
...
procedure Tt_consulta_podologo.btn_finalizarClick(Sender: TObject);
var
  st:String;
  a_guia:integer;
begin
...
procedure Tt_consulta_podologo.FormShow(Sender: TObject);
begin
  ...
  if DMC.qr_verifica_medico.RecordCount > 0 then
   begin
     ...
     guia:=strtoint(t_podologo_agenda.db_txt_guia.Text);
     CarregaDados(key,guia);
...
procedure Tt_consulta_podologo.MenuItem1Click(Sender: TObject);
begin
  t_podologo_orcamento.ShowModal;
end;

procedure Tt_consulta_podologo.MenuItem2Click(Sender: TObject);
begin
  t_podologo_materiais.ShowModal;
end;
