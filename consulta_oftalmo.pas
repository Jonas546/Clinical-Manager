...
type
  ...
  Tt_consulta_oftalmo = class(TForm)
...
var
  t_consulta_oftalmo: Tt_consulta_oftalmo;
...
{$R *.lfm}
uses dm, home, oftalmo_agenda, oftalmo_orcamento, oftalmo_materiais;

procedure CarregaDados(key,guia:integer);
begin

  DMC.query_oftalmo.Close;
  DMC.query_oftalmo.ParamByName('v1').AsInteger:=key;
  DMC.query_oftalmo.ParamByName('v2').AsInteger:=guia;
  DMC.query_oftalmo.Open;

end;

{ Tt_consulta_oftalmo }

procedure Tt_consulta_oftalmo.FormCreate(Sender: TObject);
begin
...
procedure Tt_consulta_oftalmo.FormClose(Sender: TObject;
...
procedure Tt_consulta_oftalmo.btn_salvarClick(Sender: TObject);
var
...
begin
  ...
  if (txt_procedimento.Text <> '') and (cb_tipo.Text <> '') then
   begin
     ...
     a_guia:=strtoint(t_oftalmo_agenda.db_txt_guia.Text);
...
procedure Tt_consulta_oftalmo.Button1Click(Sender: TObject);
var
  especialidades: String;
  a_medico, a_paciente, a_guia:integer;
begin
...
procedure Tt_consulta_oftalmo.cb_lista_procedimentoChange(Sender: TObject);
begin
...
procedure Tt_consulta_oftalmo.btn_finalizarClick(Sender: TObject);
var
  st:String;
  a_guia:integer;
begin
...
procedure Tt_consulta_oftalmo.FormShow(Sender: TObject);
begin
  ...
  if DMC.qr_verifica_medico.RecordCount > 0 then
   begin
     ...
     guia:=strtoint(t_oftalmo_agenda.db_txt_guia.Text);
     CarregaDados(key,guia);
...
procedure Tt_consulta_oftalmo.MenuItem1Click(Sender: TObject);
begin
  t_oftalmo_orcamento.ShowModal;
end;

procedure Tt_consulta_oftalmo.MenuItem2Click(Sender: TObject);
begin
  t_oftalmo_materiais.ShowModal;
end;
